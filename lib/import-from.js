...
function importCustomPropertiesFromObject(object) {
	...
	for (const key in customProperties) {
		if (key === "mediaQueries") {
			customProperties[key] = customProperties[key].map(query => {
				return {
					 params: query.params,
					 rules: importCustomPropertiesFromObject({ customProperties: query.rules })
					}
			})
		}
		else {
...
>>>>>>> getting mediaqueries with :root nodes from sources
}
