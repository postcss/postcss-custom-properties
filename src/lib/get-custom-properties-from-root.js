...
const isHtmlRule = node => node.type === 'rule' && -1 !== node.selector.split(',').findIndex(item => htmlSelectorRegExp.test(item)) && Object(node.nodes).length;
const isRootRule = node => node.type === 'rule' && -1 !== node.selector.split(',').findIndex(item => rootSelectorRegExp.test(item)) && Object(node.nodes).length;
